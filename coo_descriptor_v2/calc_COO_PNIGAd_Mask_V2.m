function [ dscDat, dscIdx, dscSize ] = calc_COO_PNIGAd_Mask_V2( img, param, msk )
...
%   'p3i(16)[1,3,5]' - calculate 3-point descriptor for distance 1,3,5
    if ~isempty(msk)
        assert(size(img,1)==size(msk,1));
        assert(size(img,2)==size(msk,2));
    end
    imd=im2double(img);
...
% % % % % % % % % % % % % % % % % % %
    ...
    for ddi=1:numDst
        ...
        if ~isempty(msk)
            catIGA=catIGA(catMSK>0);
        end
        if param.isRetSparse
            idx=unique(catIGA);
            dsch=histc(catIGA(:),idx)/param.nump;
            dscDat=[dscDat;dsch];
            dscIdx=[dscIdx; idx*ddi];
        else
            dsch=histc(catIGA(:),0:(sizTot-1))/param.nump;
            dscDat=[dscDat;dsch];
        end
    end
